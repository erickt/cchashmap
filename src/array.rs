...
impl<V> ArrayMap<V> {
    ...
    /// ```
    pub fn entry<'a, K: 'a>(&'a mut self, key: K) -> Entry<'a, K, V>
        where K: Borrow<[u8]>
    {
        unsafe {
            let item = self.iter_raw()
                .find(|&(key_ptr, key_len, _)| {
                    key.borrow() == slice::from_raw_parts(key_ptr, key_len)
                });
...
impl<'a, K, V> Index<K> for ArrayMap<V>
    where K: Borrow<[u8]>
{
    type Output = V;
